    ...
    <StyledContainer>
        ...
        {cartList.map((comic) => {
          ...
          if (couponDetails.discount) {
            const discountedPrice =
              (comic.prices[0].price / 100) * couponDetails.discount;

            if (couponDetails.type === "rare" && comic.isRare) {
              comic.prices[0].price = (
                comic.prices[0].price - discountedPrice
              ).toFixed(2);
      ...
      <div>
        {cart.length === 0 ? (
          ...
          <>
            <p>
              Total: <span>${obtainTotalValue()}</span>
            </p>
            ...
            {usedCoupon ? (
              <div className="coupon-container">
                <p>
                  Coupon applied: {couponDetails.discount}% discount on{" "}
                  {couponDetails.type} comics.{" "}
                  <span onClick={toggleUsedCoupon}>Remove?</span>
                </p>
              </div>
            ) : (
              <div className="coupon-container">
                <p>
                  Has a coupon? <span onClick={toggleShowCoupon}>Insert</span>
                </p>
                {showCoupon ? (
                  <div>
                    <input
